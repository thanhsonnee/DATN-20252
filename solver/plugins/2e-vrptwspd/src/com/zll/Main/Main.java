...
//        int seed = 0;
    ...
    public static double[] test(String instanceName, int seed, double pt, long timeLimitMs) throws IOException {
...
//            bestSolution.printSolution("/heuristic/" + instanceName + "不可行");
        ...
        double elapsedSec = duration.getMinutes() * 60 + duration.getSeconds() + (double) duration.getMilliSeconds() / 1000;
        System.out.println("time cost of this search: " + elapsedSec);

        int init2ENv = 0;
        for (BigVehicle bv : iniSolution.getDepot().getDeliveryVehicles()) {
            init2ENv += bv.getSatelliteLength();
        }
        int best2ENv = 0;
        for (BigVehicle bv : bestSolution.getDepot().getDeliveryVehicles()) {
            best2ENv += bv.getSatelliteLength();
        }
        System.out.println("SUMMARY"
                + " best_cost=" + bestSolution.getTotalCost().getTotal()
                + " best_dist=" + bestSolution.getTotalCost().getDistance()
                + " best_iter=" + bestSolution.iteration
                + " best_2e_nv=" + best2ENv
                + " init_cost=" + iniSolution.getTotalCost().getTotal()
                + " init_2e_nv=" + init2ENv
                + " elapsed=" + elapsedSec);
